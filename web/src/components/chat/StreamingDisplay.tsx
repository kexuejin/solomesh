...
export function StreamingDisplay({ groupJid, isWaiting, senderName: senderNameProp = 'AI', agentId }: StreamingDisplayProps) {
  ...
  const defaultTaskLabel = t('chat.store.stream.taskFallback');
      ...
      <div className="mx-auto w-full max-w-4xl px-6 py-3">
        ...
        <div className="lg:flex lg:gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="overflow-hidden rounded-xl rounded-tl-none border border-sidebar-border bg-card px-4 py-4 shadow-[0_4px_18px_-2px_rgba(15,23,42,0.05)]">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm text-muted-foreground ml-1">{t('chat.streamingDisplay.thinking')}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="overflow-hidden rounded-xl rounded-tl-none border border-sidebar-border bg-card px-4 py-4 shadow-[0_4px_18px_-2px_rgba(15,23,42,0.05)]">
            ...
            {streaming.systemStatus && (
              <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                <span>{streaming.systemStatus === 'compacting' ? t('chat.streamingDisplay.compacting') : streaming.systemStatus}</span>
              </div>
            )}

            {/* Reasoning block */}
            {streaming.thinkingText && (
              <div className="mb-3 overflow-hidden rounded-xl border border-amber-200/70 bg-amber-50/35">
                <button
                  onClick={() => {
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-amber-50/60"
                >
                  ...
                  <span className="text-xs font-medium text-amber-700">
                    {streaming.isThinking ? t('chat.streamingDisplay.reasoningActive') : t('chat.streamingDisplay.reasoning')}
                  </span>
                  {streaming.isThinking && (
            ...
            {/* Active tools */}
            {streaming.activeTools.length > 0 && (() => {
              // Split non-teammate Task tools (inline cards) from regular tools (pills).
                ...
                <div className="mb-2 space-y-2">
                  ...
                  {inlineTaskTools.map((tool) => (
                    <TaskInlineCard
                      ...
                      description={tool.toolInputSummary || sdkTasks[tool.toolUseId]?.description || defaultTaskLabel}
                      startTime={tool.startTime}
                      groupJid={groupJid}
                    />
                  ))}

                  {/* Regular tool execution card */}
                  {pillTools.length > 0 && (
                    <div className="overflow-hidden rounded-[14px] border border-sidebar-border">
                      <div className="flex items-center justify-between border-b border-sidebar-border bg-background px-4 py-2.5">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] font-bold uppercase tracking-wider text-foreground">{t('chat.streamingDisplay.toolExecution')}</span>
                        </div>
                        <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-700">{t('chat.streamingDisplay.active')}</span>
                      </div>
                          ...
                          return (
                            <div key={tool.toolUseId || i} className={isNested ? 'border-l-2 border-brand-200 pl-3' : ''}>
                              ...
                              {tool.toolInputSummary && (
                                <div className="rounded-lg bg-slate-900 px-3 py-2 font-mono text-[11px] text-slate-300">
                                  {tool.toolInputSummary}
                                </div>
                              )}
                            </div>
            ...
            {streaming.recentEvents.length > 0 && (
              <div className="mb-2 rounded-lg border border-sidebar-border bg-background px-3 py-2">
                <div className="mb-1 text-[11px] font-medium text-muted-foreground">{t('chat.streamingDisplay.trace')}</div>
            ...
            {streaming.activeHook && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
                ...
                <span>{t('chat.streamingDisplay.hookPrefix')}: {streaming.activeHook.hookName}</span>
              </div>
            )}
