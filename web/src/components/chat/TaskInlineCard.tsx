    ...
    <div className="w-full my-1">
      ...
      <button
        ...
        className={`w-full flex items-center gap-2 rounded-lg px-3 py-2 text-left transition-colors ${
          isRunning
            ? 'border border-brand-200/80 bg-brand-50/45 hover:bg-brand-50/70'
            : status === 'completed'
              ? 'border border-emerald-200/80 bg-emerald-50/45 hover:bg-emerald-50/70'
              : 'border border-red-200/80 bg-red-50/45 hover:bg-red-50/70'
        }`}
      >
        {statusIcon}
      ...
      {expanded && hasContent && streaming && (
        <div
          ref={contentRef}
          className="mt-1 max-h-80 overflow-y-auto rounded-xl border border-sidebar-border bg-card"
        >
