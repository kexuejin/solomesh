...
const chatJid = process.env.SOLOMESH_CHAT_JID!;
const groupFolder = process.env.SOLOMESH_GROUP_FOLDER!;
const _isHome = process.env.SOLOMESH_IS_HOME === '1';
const isAdminHome = process.env.SOLOMESH_IS_ADMIN_HOME === '1';
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name === PRIMARY_MEMORY_FILE_NAME || MEMORY_EXTENSIONS.has(path.extname(entry.name))) {
          out.push(fullPath);
        }
      }
    }
...
**重要**：下次对话仍可能用到的信息（用户身份、偏好、常用项目、用户说"记住"的内容）应直接用 Edit 工具编辑 /workspace/global/${PRIMARY_MEMORY_FILE_NAME}，不要用此工具。`,
  {
...
server.tool(
  'memory_search',
  `在工作区的记忆文件中搜索（${PRIMARY_MEMORY_FILE_NAME}、memory/、conversations/ 及其他 .md/.txt 文件）。
...
server.tool(
  ...
  {
    file: z.string().describe(`相对路径，可带 :行号（如 "${PRIMARY_MEMORY_FILE_NAME}:12"、"[global] ${PRIMARY_MEMORY_FILE_NAME}:8" 或 "[memory] 2026-01-15.md"）`),
