...
function syncRuntimePrimaryMemoryFiles(baseDir: string): void {
  try {
    if (!fs.existsSync(baseDir)) return;

    const memoryFilePaths = RUNTIME_PRIMARY_MEMORY_FILE_NAMES.map((name) =>
      path.join(baseDir, name),
    );
    const existingPaths = memoryFilePaths.filter((p) => fs.existsSync(p));
    if (existingPaths.length === 0) return;

    let sourcePath = existingPaths[0];
    if (existingPaths.length > 1) {
      const sorted = [...existingPaths].sort((a, b) => {
    ...
    for (const filePath of memoryFilePaths) {
      try {
        ...
        // Some mounts are read-only (non-home /workspace/global); skip sync silently.
      }
    }
  } catch {
...
const MEMORY_FLUSH_ALLOWED_TOOLS = [
  'mcp__solomesh__memory_search',
  'mcp__solomesh__memory_get',
  'mcp__solomesh__memory_append',
  'Read',  // 读取全局记忆文件当前内容
  'Edit',  // 编辑全局记忆文件（永久记忆）
];
...
const MEMORY_FLUSH_DISALLOWED_TOOLS = [
  ...
  'mcp__solomesh__send_message',
  'mcp__solomesh__schedule_task',
  'mcp__solomesh__list_tasks',
  'mcp__solomesh__pause_task',
  'mcp__solomesh__resume_task',
  'mcp__solomesh__cancel_task',
  'mcp__solomesh__register_group',
];
...
const OUTPUT_START_MARKER = '---SOLOMESH_OUTPUT_START---';
const OUTPUT_END_MARKER = '---SOLOMESH_OUTPUT_END---';
...
function formatTranscriptMarkdown(messages: ParsedMessage[], title?: string | null): string {
  ...
  for (const msg of messages) {
    const sender = msg.role === 'user' ? 'User' : 'SoloMesh';
...
function buildMemoryRecallPrompt(isHome: boolean, isAdminHome: boolean): string {
  if (isHome) {
    // Home container (admin or member): full memory system with read/write access to user's global memory file
    return [
      '',
      '## 记忆系统',
      '',
      ...
      '根据信息的**时效性**选择存储位置：',
      '',
      `#### 全局记忆（永久）→ 直接编辑 \`/workspace/global/${PRIMARY_MEMORY_FILE_NAME}\``,
      '',
      ...
      `> - 是 / 可能 → **全局记忆**（编辑 \`/workspace/global/${PRIMARY_MEMORY_FILE_NAME}\`）`,
  ...
  return [
    ...
    '不要等待——获知后立刻存储。',
    `全局记忆（\`/workspace/global/${PRIMARY_MEMORY_FILE_NAME}\`）为只读，无法直接修改。`,
  ].join('\n');
}
 ...
 */
async function loadClaudeSdk(): Promise<{
  query: (
    input: Record<string, unknown>,
  ) => AsyncIterable<any> & { interrupt(): Promise<void> };
}> {
  try {
    ...
    throw new Error(
      `Claude Agent SDK 未安装或加载失败: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
async function runClaudeQuery(
  ...
  // Build system prompt: memory recall guidance + global memory file (for non-admin-home)
  const { isHome, isAdminHome } = normalizeHomeFlags(containerInput);
  syncRuntimePrimaryMemoryFiles(WORKSPACE_GLOBAL);
  syncRuntimePrimaryMemoryFiles(WORKSPACE_GROUP);
  const globalMemoryPath = path.join(WORKSPACE_GLOBAL, PRIMARY_MEMORY_FILE_NAME);

  // Always inject global memory content into system prompt so the agent
  ...
  if (fs.existsSync(globalMemoryPath)) {
    globalMemory = fs.readFileSync(globalMemoryPath, 'utf-8');
  }
  ...
  // Non-home containers only access memory (global memory file injected via systemPromptAppend)
...
async function loadCodexSdk(): Promise<{ Codex: new (options?: Record<string, unknown>) => any }> {
  try {
    return (await Function('return import("@openai/codex-sdk")')()) as {
      Codex: new (options?: Record<string, unknown>) => any;
    };
  } catch (err) {
    throw new Error(
      `Codex SDK 未安装或加载失败: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
}
...
function getErrorMessage(value: unknown): string | undefined {
  ...
  return getStringField(obj, ['message', 'error']);
}

function toAsyncEventStream(result: unknown): AsyncIterable<unknown> {
  const direct = result as { [Symbol.asyncIterator]?: () => AsyncIterator<unknown> };
  if (direct && typeof direct[Symbol.asyncIterator] === 'function') {
    return direct as AsyncIterable<unknown>;
  }

  const wrapped = asRecord(result);
  const events = wrapped?.events as { [Symbol.asyncIterator]?: () => AsyncIterator<unknown> } | undefined;
  if (events && typeof events[Symbol.asyncIterator] === 'function') {
    return events as AsyncIterable<unknown>;
  }

  throw new Error('Codex SDK runStreamed 返回值无法迭代');
}

function normalizeCodexBaseUrlForSdk(raw: string): string {
  try {
...
async function runCodexQuery(
  ...
  if (process.env.OPENAI_BASE_URL) {
    codexOptions.baseUrl = normalizeCodexBaseUrlForSdk(
  ...
  let newSessionId: string | undefined =
    getStringField(threadRecord, ['id', 'thread_id', 'threadId']) || sessionId;
  let lastAssistantUuid: string | undefined;
  let closedDuringQuery = false;
  let interruptedDuringQuery = false;
  let fullText = '';
  ...
  try {
    ...
    for await (const event of stream) {
      ...
      const eventType = getStringField(eventObj, ['type']);
      if (eventType === 'thread.started') {
        newSessionId =
          getStringField(eventObj, ['thread_id', 'threadId']) || newSessionId;
        continue;
      }

      if (eventType === 'item.started') {
        const item = asRecord(eventObj?.item);
        const itemType = getStringField(item, ['type']);
        const itemId =
          getStringField(item, ['id']) ||
          getStringField(eventObj, ['item_id', 'itemId']);
        if (itemType === 'command_execution') {
          emit({
            ...
            streamEvent: {
              ...
              toolUseId: itemId,
              toolName: 'Bash',
              toolInputSummary: getStringField(item, ['command']),
            },
          });
        } else if (itemType === 'mcp_tool_call') {
          const server = getStringField(item, ['server']) || 'mcp';
          const tool = getStringField(item, ['tool']) || 'tool';
          emit({
            status: 'stream',
            result: null,
            streamEvent: {
          ...
          emit({
            ...
            streamEvent: {
              ...
              toolUseId: itemId,
              toolName: 'WebSearch',
              toolInputSummary: getStringField(item, ['query']),
            },
          });
        }
        continue;
      ...
      if (eventType === 'item.updated') {
        ...
        const aggregatedOutput =
          getStringField(item, ['aggregated_output', 'aggregatedOutput']) ||
          getStringField(item, ['output']);
        if (
          getStringField(item, ['type']) === 'command_execution' &&
          itemId &&
          typeof aggregatedOutput === 'string'
        ) {
          const prev = commandOutputCache.get(itemId) || '';
          const current = aggregatedOutput;
          ...
          commandOutputCache.set(itemId, current);
        }
        continue;
      }
      ...
      if (eventType === 'item.completed') {
        ...
        if (itemType === 'command_execution') {
          ...
          const finalOutput =
            ...
            getStringField(item, ['output']) ||
            '';
          if (cmdId && finalOutput) {
            const prev = commandOutputCache.get(cmdId) || '';
            if (finalOutput.length > prev.length) {
              emit({
                status: 'stream',
                result: null,
                streamEvent: {
                  eventType: 'tool_progress',
                  toolUseId: cmdId,
                  text: finalOutput.slice(prev.length),
                },
              });
            }
            commandOutputCache.set(cmdId, finalOutput);
          }
          emit({
            status: 'stream',
            result: null,
            streamEvent: { eventType: 'tool_use_end', toolUseId: cmdId },
          });
          continue;
        }

        if (itemType === 'mcp_tool_call') {
          const toolUseId = itemId;
          const output = getStringField(item, ['output']);
          const errorMsg = getErrorMessage((item as Record<string, unknown>)?.error);
          const text = output || errorMsg;
          if (toolUseId && text) {
            emit({
              status: 'stream',
              result: null,
        ...
        if (itemType === 'web_search') {
          emit({
            status: 'stream',
            result: null,
            streamEvent: { eventType: 'tool_use_end', toolUseId: itemId },
          });
          continue;
        }

        const agentText = getStringField(item, ['text']);
        if (itemType === 'agent_message' && agentText) {
          const delta = agentText.startsWith(fullText)
            ? agentText.slice(fullText.length)
            : agentText;
          fullText = agentText;
          lastAssistantUuid = itemId || lastAssistantUuid;
          emit({
            status: 'stream',
            result: null,
            streamEvent: { eventType: 'text_delta', text: delta || agentText },
          });
          continue;
        }

        if (itemType === 'reasoning') {
          const thinking = agentText?.trim() || '';
          if (thinking) {
            emit({
              status: 'stream',
              result: null,
              streamEvent: { eventType: 'thinking_delta', text: `${thinking}\n` },
            });
          }
        }
        continue;
      ...
      if (eventType === 'turn.failed') {
        const turn = asRecord(eventObj?.turn);
        const message =
          getErrorMessage(turn?.error) ||
          getStringField(turn, ['error']) ||
          'Codex turn failed';
      ...
      if (eventType === 'error') {
        const message =
          getStringField(eventObj, ['message']) ||
          getErrorMessage(eventObj?.error) ||
          'Codex stream error';
        throw new Error(message);
      }
    }
...
async function main(): Promise<void> {
  ...
  const activeProviderBase = AGENT_PROVIDERS[AGENT_PROVIDER_ID];
  const activeProvider: AgentProviderRuntime = {
    ...activeProviderBase,
    label:
      process.env.SOLOMESH_RUNTIME_LABEL?.trim() ||
      activeProviderBase.label,
    capabilities: {
      ...activeProviderBase.capabilities,
      supportsMemoryFlush: parseBooleanEnv(
        'SOLOMESH_CAP_SUPPORTS_MEMORY_FLUSH',
        activeProviderBase.capabilities.supportsMemoryFlush,
      ),
    },
  };
  const queryRunner = activeProvider.run;
  log(`Agent provider: ${activeProvider.id} (${activeProvider.label})`);
  ...
  try {
    while (true) {
      ...
      if (
        activeProvider.capabilities.supportsMemoryFlush &&
        needsMemoryFlush &&
        isAdminHome
      ) {
        ...
        const flushPrompt = [
          '上下文压缩前记忆刷新。',
          `**优先检查全局记忆**：先 Read /workspace/global/${PRIMARY_MEMORY_FILE_NAME}，如果有「待记录」字段且你已获知对应信息（用户身份、偏好、常用项目等），用 Edit 工具立即填写。`,
        ...
        const flushResult = await queryRunner(
          flushPrompt,
          sessionId,
          mcpServerPath,
          containerInput,
