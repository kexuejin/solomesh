...
const CORS_ALLOW_PRIVATE_NETWORK =
  process.env.CORS_ALLOW_PRIVATE_NETWORK !== 'false'; // default: true
...
function isPrivateHostname(hostname: string): boolean {
  ...
  if (net.isIPv6(cleaned)) {
    ...
    if (cleaned.startsWith('fd') || cleaned.startsWith('fc')) return true; // unique local
...
function isAllowedOrigin(origin: string | undefined, requestHost?: string): string | null {
  ...
  try {
    ...
    // Same host but different port/protocol (common in dev setups)
    const reqHost = (requestHost || '').split(':')[0]?.toLowerCase();
    if (reqHost && url.hostname.toLowerCase() === reqHost) return origin;
    ...
    // 允许私有网络来源（常见于局域网 IP 访问）
    if (CORS_ALLOW_PRIVATE_NETWORK && isPrivateHostname(url.hostname)) {
      return origin;
    }
  } catch {
    // invalid origin
...
app.use(
  '/api/*',
  cors({
    origin: (origin, c) => isAllowedOrigin(origin, c.req.header('host') || undefined),
    credentials: true,
  }),
);
...
> {
  ...
  const formatted = deps.formatMessages([
    {
      ...
      sender: userId,
      sender_name: displayName,
      content: providerDirective.contentForPrompt,
      timestamp,
    },
  ], shared);
  ...
  if (group.is_home || providerDirective.hasDirective || hasWorkflowControl) {
    deps.queue.closeStdin(chatJid);
    if (providerDirective.hasDirective) {
      logger.debug({ chatJid }, 'Provider directive detected in message, forcing new run');
    }
    if (hasWorkflowControl) {
      logger.debug(
        { chatJid, command: workflowCommand.type },
        'Workflow command detected in message, forcing new run',
      );
    }
...
async function handleAgentConversationMessage(
  ...
  // Provider directive requires a fresh run; existing process keeps its original provider.
  if (providerDirective.hasDirective || hasWorkflowControl) {
    deps.queue.closeStdin(virtualChatJid);
    if (providerDirective.hasDirective) {
      logger.debug(
        { chatJid, agentId },
        'Provider directive detected in agent conversation, forcing new run',
      );
    }
    if (hasWorkflowControl) {
      logger.debug(
        { chatJid, agentId, command: workflowCommand.type },
        'Workflow command detected in agent conversation, forcing new run',
      );
    }
  }
...
function setupWebSocket(server: any): WebSocketServer {
  ...
  server.on('upgrade', (request: any, socket: any, head: any) => {
    ...
    request.__solomeshSessionId = token;
  ...
  wss.on('connection', (ws, request: any) => {
    const sessionId = request?.__solomeshSessionId as string | undefined;
