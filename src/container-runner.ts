...
function buildVolumeMounts(
  ...
  const mergedConfig = mergeRuntimeEnvConfig(globalConfig, containerOverride);
...
export async function runContainerAgent(
  ...
  const containerName = `solomesh-${safeName}${agentSuffix}-${Date.now()}`;
...
export async function runHostAgent(
  ...
  const groupSessionsDir = input.agentId
    ...
    : path.join(DATA_DIR, 'sessions', group.folder, '.claude');
  fs.mkdirSync(groupSessionsDir, { recursive: true });
  const groupCodexDir = input.agentId
  ...
  if (ownerId) {
    ...
    hostEnv['SOLOMESH_WORKSPACE_GLOBAL'] = userGlobalDir;
  } else {
    const legacyGlobalDir = path.join(GROUPS_DIR, 'global');
    fs.mkdirSync(legacyGlobalDir, { recursive: true });
    hostEnv['SOLOMESH_WORKSPACE_GLOBAL'] = legacyGlobalDir;
  }
  hostEnv['SOLOMESH_WORKSPACE_MEMORY'] = path.join(DATA_DIR, 'memory', group.folder);
