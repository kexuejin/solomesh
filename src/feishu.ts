...
export function createFeishuConnection(config: FeishuConnectionConfig): FeishuConnection {
  ...
  const CHAT_NAME_CACHE_TTL = 30 * 60 * 1000; // 30min
  ...
  function getFallbackChatName(chatType: string): string {
    return chatType === 'p2p' ? '飞书私聊' : '飞书群聊';
  }

  function isFallbackChatName(name: string, chatJid: string): boolean {
  ...
  function cacheChatName(chatId: string, rawName: string): void {
    const name = rawName.trim();
    if (!name) return;
    chatNameCache.set(chatId, {
      name,
      expiresAt: Date.now() + CHAT_NAME_CACHE_TTL,
    });
  }

  function getCachedChatName(chatId: string): string | null {
  ...
  async function resolveChatName(chatId: string, chatType: string): Promise<string> {
    const cachedName = getCachedChatName(chatId);
    if (cachedName) return cachedName;

    const fallbackName = getFallbackChatName(chatType);
    if (!client || chatType === 'p2p') {
      return fallbackName;
    }
    ...
    try {
      ...
      const resolvedName = res.data?.name?.trim() || '';
      if (resolvedName) {
        cacheChatName(chatId, resolvedName);
        return resolvedName;
      }
    } catch (err) {
      logger.debug({ err, chatId }, 'Failed to resolve Feishu chat name');
    }

    return fallbackName;
  }

  async function backfillKnownChatNames(): Promise<void> {
    if (!client) return;
    let knownChats: ReturnType<typeof getAllChats>;
    try {
      knownChats = getAllChats();
    } catch (err) {
      logger.debug({ err }, 'Skip Feishu chat name backfill before DB initialization');
      return;
    }

    const staleChats = knownChats
      .filter((chat) => chat.jid.startsWith('feishu:'))
      .filter((chat) => isFallbackChatName(chat.name, chat.jid));
    if (staleChats.length === 0) return;

    let updatedCount = 0;
    for (const chat of staleChats) {
      const chatId = chat.jid.slice('feishu:'.length);
      if (!chatId) continue;
      try {
        const res = await client.im.v1.chat.get({
          path: { chat_id: chatId },
        ...
        cacheChatName(chatId, resolvedName);
        updateChatName(chat.jid, resolvedName);
        updatedCount += 1;
      } catch (err) {
        logger.debug({ err, chatId }, 'Failed to backfill Feishu chat name');
      }
    }

    if (updatedCount > 0) {
      logger.info({ updatedCount }, 'Backfilled Feishu chat names');
    }
  }
  ...
  const connection: FeishuConnection = {
    async connect(opts: ConnectOptions): Promise<boolean> {
      ...
      const eventDispatcher = new lark.EventDispatcher({}).register({
        'im.message.receive_v1': async (data) => {
          try {
            ...
            const chatName = await resolveChatName(chatId, message.chat_type);
      ...
      try {
        ...
        void backfillKnownChatNames().catch((err) =>
          logger.warn({ err }, 'Feishu chat name backfill failed'),
        );
        onReady();
        return true;
      } catch (err) {
        ...
        while (hasMore) {
          ...
          for (const chat of items) {
            if (chat.chat_id && chat.name) {
              cacheChatName(chat.chat_id, chat.name);
