...
function getActivePrimaryMemoryFileNames(): string[] {
  return listRuntimePrimaryMemoryFileNames(
...
function classifyMemorySource(
  ...
  if (parts[0] === 'data' && parts[1] === 'groups' && parts[2] === 'user-global') {
    ...
    const name = parts.slice(4).join('/') || primaryMemoryFileName;
    const owner = getUserById(userId);
    const ownerLabel = owner ? (owner.display_name || owner.username) : userId;
    return {
      scope: 'user-global',
      kind: isRuntimePrimaryMemoryFileName(name) ? 'primary' : 'note',
      label: `${ownerLabel} / 全局记忆 / ${name}`,
      ownerName: ownerLabel,
    };
  }
  // data/groups/main/{provider-memory-file}
  if (parts[0] === 'data' && parts[1] === 'groups' && parts[2] === 'main') {
    const name = parts.slice(3).join('/') || primaryMemoryFileName;
    if (isRuntimePrimaryMemoryFileName(name)) {
      return {
        scope: 'main',
        kind: 'primary',
        label: `主会话记忆 / ${name}`,
  ...
  if (parts[0] === 'data' && parts[1] === 'groups') {
    ...
    const kind: MemorySource['kind'] = isRuntimePrimaryMemoryFileName(name)
      ? 'primary'
      : 'note';
    return {
      scope: folder === 'main' ? 'main' : 'flow',
...
// 记忆路径中禁止写入的系统子目录（主记忆文件除外，它是允许编辑的记忆文件）
...
function listMemorySources(user: AuthUser): MemorySource[] {
  const primaryMemoryFileNames = getActivePrimaryMemoryFileNames();
  ...
  if (isAdmin) {
      ...
      for (const d of userDirs) {
        if (d.isDirectory()) {
          for (const fileName of primaryMemoryFileNames) {
            files.add(path.join(USER_GLOBAL_DIR, d.name, fileName));
          }
        }
      }
    }
    ...
    for (const fileName of primaryMemoryFileNames) {
      files.add(path.join(USER_GLOBAL_DIR, user.id, fileName));
    }
  }
  ...
  for (const folder of accessibleFolders) {
    for (const fileName of primaryMemoryFileNames) {
      files.add(path.join(GROUPS_DIR, folder, fileName));
    }
  }
