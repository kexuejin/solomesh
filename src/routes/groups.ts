...
import { resolveMergedMessageQueryJids } from '../group-message-merge.js';
import {
  type AgentProvider,
...
function isImFallbackName(name: string | null | undefined, jid: string): boolean {
...
interface GroupPayloadItem {
  ...
  workflow_status?: WorkflowSessionState['status'];
  workflow_template_id?: string | null;
  workflow_stage_id?: string | null;
  workflow_stage_name?: string | null;
  workflow_stage_provider?: AgentProvider | null;
  workflow_stage_default_provider?: AgentProvider | null;
  workflow_stage_fallback_from_provider?: AgentProvider | null;
...
function loadChatProviderSelections(): Record<string, AgentProvider> {
  const raw = getRouterState('chat_provider_selections');
  if (!raw) return {};
...
function loadChatWorkflowStates(): Record<string, WorkflowSessionState> {
  const raw = getRouterState('chat_workflow_state');
  return parseWorkflowSessionMap(raw);
}
...
function buildGroupsPayload(user: AuthUser): Record<string, GroupPayloadItem> {
  ...
  const folderCandidates = new Map<string, GroupWithJid[]>();
  for (const [jid, group] of Object.entries(groups)) {
    const list = folderCandidates.get(group.folder) || [];
    list.push({ jid, ...group });
    folderCandidates.set(group.folder, list);
  }
  const chats = new Map(getAllChats().map((chat) => [chat.jid, chat]));
  const bindingMap = new Map(
    listChannelSessionBindings(
      user.role === 'admin' ? {} : { ownerUserId: user.id },
    ).map((binding) => [binding.chat_jid, binding]),
  );
  const globalProviderConfig = getRuntimeProviderConfig();
  const effectiveProviderCache = new Map<string, AgentProvider>();
  const chatProviderSelections = loadChatProviderSelections();
  const chatWorkflowStates = loadChatWorkflowStates();
  ...
  const resolveEffectiveProvider = (folder: string): AgentProvider => {
    const cached = effectiveProviderCache.get(folder);
    if (cached) return cached;
    const merged = mergeRuntimeEnvConfig(
      globalProviderConfig,
      getContainerEnvConfig(folder),
    );
    effectiveProviderCache.set(folder, merged.agentRuntime);
    return merged.agentRuntime;
  };
  ...
  for (const [jid, group] of visibleEntries) {
    const isHome = !!group.is_home;
    const isWeb = jid.startsWith('web:');
    const imChannel = isHome || isWeb ? null : parseImChannelFromJid(jid);
    const binding = imChannel ? bindingMap.get(jid) : undefined;
    const chatInfo = chats.get(jid);
    const chatNameCandidate = chatInfo?.name || '';
    const groupNameCandidate = group.name || '';
    const resolvedName = imChannel
      ? (
        !isImFallbackName(chatNameCandidate, jid)
          ? chatNameCandidate
          : groupNameCandidate
      )
      : groupNameCandidate;
    ...
    const workflowState = chatWorkflowStates[jid] ?? null;
    const workflowTemplate = getWorkflowTemplateForSession(workflowState);
    const workflowStage = workflowState && workflowTemplate
      ? workflowTemplate.stages[workflowState.currentStageIndex] ?? null
      : null;
    const workflowMetadata = (
      ...
      : {};
    const activeFallbackProvider = normalizeAgentProvider(workflowMetadata.activeFallbackProvider);
    const activeFallbackStageId = typeof workflowMetadata.activeFallbackStageId === 'string'
      ? workflowMetadata.activeFallbackStageId
      : null;
    const activeFallbackFromProvider = normalizeAgentProvider(workflowMetadata.activeFallbackFromProvider);
    const workflowStageProvider = workflowStage
      ? (
        activeFallbackProvider && activeFallbackStageId === workflowStage.id
          ? activeFallbackProvider
          : workflowStage.defaultProvider
      )
      : null;
    const workflowStageTotal = workflowTemplate?.stages.length ?? null;
    const workflowStageIndex = workflowState && workflowStageTotal
      ? Math.min(workflowStageTotal, Math.max(1, workflowState.currentStageIndex + 1))
      : null;
    const selectedProvider =
      workflowStageProvider
      ?? chatProviderSelections[jid]
      ?? resolveEffectiveProvider(group.folder);
    const executionMode = resolveExecutionModeForGroup(
      { jid, ...group },
      folderCandidates.get(group.folder) || [{ jid, ...group }],
    );
    ...
    result[jid] = {
      ...
      kind: isHome ? 'home' : isWeb ? 'web' : (imChannel ?? 'feishu'),
      editable: isWeb,
      ...
      effective_provider: selectedProvider,
      workflow_status: workflowState?.status,
      workflow_template_id: workflowState?.templateId ?? null,
      workflow_stage_id: workflowStage?.id ?? null,
      workflow_stage_name: workflowStage?.name ?? null,
      workflow_stage_provider: workflowStageProvider,
      workflow_stage_default_provider: workflowStage?.defaultProvider ?? null,
      workflow_stage_fallback_from_provider:
        workflowStageProvider
        && workflowStage
        && workflowStageProvider !== workflowStage.defaultProvider
          ? (activeFallbackFromProvider ?? workflowStage.defaultProvider)
          : null,
      workflow_stage_index: workflowStageIndex,
      workflow_stage_total: workflowStageTotal,
      workflow_blocked_by:
        typeof workflowMetadata.blockedBy === 'string'
          ? workflowMetadata.blockedBy
          : null,
      workflow_blocked_reason:
        typeof workflowMetadata.blockedReason === 'string'
          ? workflowMetadata.blockedReason
          : null,
      im_binding_enabled:
        imChannel && binding ? binding.enabled : undefined,
      im_binding_target_folder:
        imChannel && binding ? binding.target_folder : null,
      im_binding_updated_at:
        imChannel && binding ? binding.updated_at : null,
    };
  }
...
function clearSessionJsonlFiles(folder: string): void {
  const sessionsRoot = path.join(DATA_DIR, 'sessions', folder);
  for (const target of listSessionCleanupPlan(sessionsRoot)) {
...
function resetWorkspaceForGroup(folder: string): void {
  // 1. 清除工作目录（Agent 文件、记忆文件、logs/ 等），然后重建空目录
...
// PATCH /api/groups/:jid - 更新群组元数据（名称、技能、宿主机目录）
groupRoutes.patch('/:jid', authMiddleware, async (c) => {
  ...
  const customCwdProvided = Object.prototype.hasOwnProperty.call(body, 'custom_cwd');
  const { name: rawName, selected_skills } = validation.data;
  const rawCustomCwd = validation.data.custom_cwd;
  const name = rawName ? normalizeGroupName(rawName) : undefined;
  const customCwdInput = typeof rawCustomCwd === 'string' ? rawCustomCwd.trim() : rawCustomCwd;

  let nextCustomCwd = existing.customCwd;
  if (customCwdProvided) {
    if (!hasHostExecutionPermission(authUser)) {
      return c.json(
        { error: 'Insufficient permissions for host execution mode' },
        403,
    ...
    if ((existing.executionMode || 'container') !== 'host') {
      return c.json({ error: 'custom_cwd is only valid for host mode' }, 400);
    }

    // null / empty => clear custom cwd and fallback to default group directory.
    if (customCwdInput == null || customCwdInput === '') {
      nextCustomCwd = undefined;
    } else {
      if (!path.isAbsolute(customCwdInput)) {
        return c.json({ error: 'custom_cwd must be an absolute path' }, 400);
      }

      // Validate target directory and canonicalize symlink path.
      let realPath: string;
      try {
        const stat = fs.statSync(customCwdInput);
        if (!stat.isDirectory()) {
          return c.json(
            { error: 'custom_cwd must be an existing directory' },
            400,
        ...
        realPath = fs.realpathSync(customCwdInput);
      } catch {
        return c.json({ error: 'custom_cwd directory does not exist' }, 400);
      }
      ...
      nextCustomCwd = realPath;
    }
  }

  // 至少需要提供一个字段
  if (!name && selected_skills === undefined && !customCwdProvided) {
    return c.json({ error: 'No fields to update' }, 400);
  }

  const customCwdChanged = customCwdProvided
    && (nextCustomCwd || '') !== (existing.customCwd || '');
  if (customCwdChanged) {
    const siblingJids = getJidsByFolder(existing.folder);
    try {
      // Ensure no process keeps running in old cwd.
      ...
      logger.error(
        ...
        'Failed to stop group while updating custom cwd',
      );
      return c.json(
        { error: 'Failed to stop running process, custom_cwd not updated' },
        500,
      );
    }
  }
...
groupRoutes.post('/:jid/interrupt', authMiddleware, async (c) => {
  ...
  const hasUnansweredUserTurn = !!latest
    && !latest.is_from_me
    && latest.sender !== '__system__';
  const shouldAppendMarker = interrupted || hasUnansweredUserTurn;

  if (shouldAppendMarker) {
    // Persist interrupt marker so UI can deterministically clear waiting state.
    ...
    try {
      ...
      logger.warn(
        { jid, interrupted, hasUnansweredUserTurn, err },
        'Interrupt requested but failed to append system marker',
      );
    }
  }
...
groupRoutes.get('/:jid/messages', authMiddleware, async (c) => {
  ...
  if (agentIdParam) {
    ...
    if (after) {
      const messages = getMessagesAfter(virtualJid, after, limit, afterId);
  ...
  // Message merge scope:
  // - home: preserve owner-scoped sibling merge behavior
  // - non-home web workspace: merge accessible IM siblings in same folder
  const queryJids = resolveMergedMessageQueryJids({
    target: group,
    viewer: { id: authUser.id, role: authUser.role },
    siblings,
    canAccess: (candidate) =>
      canAccessGroup(
  ...
  if (queryJids.length === 1) {
    // 单 JID 走原路径
    if (after) {
      const messages = getMessagesAfter(jid, after, limit, afterId);
      return c.json({ messages });
    }
  ...
  // 多 JID 合并查询
  if (after) {
    const messages = getMessagesAfterMulti(queryJids, after, limit, afterId);
    return c.json({ messages });
  }
...
groupRoutes.put('/:jid/env', authMiddleware, async (c) => {
  ...
  if (data.agentRuntime !== undefined)
    updated.agentRuntime = data.agentRuntime;
  if (data.codexBaseUrl !== undefined)
