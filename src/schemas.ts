...
import { z } from 'zod';
import { AGENT_PROVIDER_IDS } from './agent-providers.js';
 ...
 */
export const RuntimeModelSelectionSchema = z
  .object({
    agentRuntime: AgentRuntimeIdSchema.optional(),
    modelProvider: ModelProviderIdSchema.optional(),
    model: z.string().min(1).max(256).optional(),
  })
  .strict()
  .refine(
    (data) =>
      typeof data.agentRuntime === 'string' ||
...
export const RuntimeConfigSchema = z
  ...
  .refine(
    (data) =>
      ...
      typeof data.codexBaseUrl === 'string' ||
      typeof data.codexModel === 'string' ||
      typeof data.geminiBaseUrl === 'string' ||
      typeof data.geminiModel === 'string' ||
      typeof data.geminiAuthMode === 'string',
    { message: 'At least one config field must be provided' },
  );

export const GroupPatchSchema = z.object({
  name: z.string().min(1).max(MAX_GROUP_NAME_LEN).optional(),
  custom_cwd: z.union([z.string().max(2000), z.null()]).optional(),
  selected_skills: z.array(z.string().max(128)).max(200).nullable().optional(),
});

export const ChannelSessionBindingUpsertSchema = z.object({
  chatJid: z.string().min(1),
  targetFolder: z.string().min(1).max(256),
...
export const RuntimeOAuthCredentialsSchema = z.object({
...
export const RuntimeCustomEnvSchema = z.object({
